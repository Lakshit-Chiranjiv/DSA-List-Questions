...
public class Q18{
    // Count partition with given differences

    //will work for all arr[i] > 0

    // recursive solution - Time: O(2^n) Space: O(n)
    public static int countSubsetSum(int[] arr, int idx, int target) {
    ...
    public static int countPartitionMemo(int[] arr, int n, int diff, int[][] dp){
        ...
        for(int i=0; i<n; i++)
            sum += arr[i];
        int s2 = (sum - diff)/2;
        return countSubsetSumMemo(arr, n, s2, dp);
    }
    ...
    public static int countPartitionTab(int[] arr, int n, int diff){
        ...
        for(int i=0; i<n; i++)
            sum += arr[i];
        int s2 = (sum - diff)/2;
        return countSubsetSumTab(arr, s2);
    }
    ...
    public static int countPartitionTabSpaceOpt(int[] arr, int n, int diff){
        ...
        for(int i=0; i<n; i++)
            sum += arr[i];
        int s2 = (sum - diff)/2;
        return countSubsetSumTabSpaceOpt(arr, s2);
    }
}